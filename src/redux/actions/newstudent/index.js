...
//   };
// };
export const ADD_NOTE_FOR_STUDENT = (
  payload,
  studentId,
  studentType,
  dateTill
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      dispatch(GET_NOTES_BY_STUDENT(studentId));
      if (response?.data?.success) {
        if (payload.noteType === "Miss You") {
          ...
          dispatch(GET_MISS_YOU_CALL(0, 10, studentType, dateTill));
        } else if (payload.noteType === "Renewal") {
          dispatch(
            GET_NOTES_BY_MEMBERID("Renewal", "today", 0, 10),
            GET_NOTES_BY_STUDENT(studentId)
          );
          dispatch(GET_RENEWAL_ALL_DATA(0, 10, studentType, dateTill));
        } else if (payload.noteType === "Birthday") {
          dispatch(
            GET_NOTES_BY_MEMBERID("Birthday", "today", 0, 10),
            GET_NOTES_BY_STUDENT(studentId)
          );
          dispatch(GET_BIRTHDAY_ALL_DATA(0, 10, studentType, dateTill));
        }
      }
...
export const EDIT_NOTE_OF_STUDENT = (payload, note) => {
  let url = `${baseUrl}/api/followup_note/update_note/${getUserId()}/${
    note?._id
  }`;
  return async (dispatch) => {
    try {
...
export const DELETE_NOTE_BY_STUDENTID = (
  note,
  studentType,
  dateTill,
  noteType,
  ...
  let url = `${baseUrl}/api/followup_note/remove_note/${getUserId()}/${
    note?._id
  }`;
  return async (dispatch) => {
    try {
      ...
      if (noteType === "Miss You") {
        dispatch(GET_NOTES_BY_MEMBERID("Miss You", "today", 0, 10));
        dispatch(GET_NOTES_BY_STUDENT(note?.memberId));
        dispatch(GET_MISS_YOU_CALL(0, 10, studentType, dateTill));
      } else if (noteType === "Renewal") {
        dispatch(GET_NOTES_BY_MEMBERID("Renewal", "today", 0, 10));
        dispatch(GET_NOTES_BY_STUDENT(note?.memberId));
        dispatch(GET_RENEWAL_ALL_DATA(0, 10, studentType, dateTill));
      } else if (noteType === "Birthday") {
        dispatch(GET_NOTES_BY_MEMBERID("Birthday", "today", 0, 10));
        dispatch(GET_NOTES_BY_STUDENT(note?.memberId));
        dispatch(GET_BIRTHDAY_ALL_DATA(0, 10, studentType, dateTill));
      }
  ...
  return async (dispatch) => {
    try {
      let response = await axios.get(
        `${baseUrl}/api/recomend_candidate/get_by_user_id/${getUserId()}/${page}/${
          sortBy === undefined ? perrow : perrow + params
        }`,
        {
          headers: getHeaders(),
        }
...
export const FILTER_LEAD_BY_SOURCE = (payload) => {
  let url = `${baseUrl}/api/students/leads/${getUserId()}?year=${
    payload.year
  }&month=${payload.month}`;
  return async (dispatch) => {
    try {
      dispatch({
        type: "FILTER_LEAD_BY_SOURCE",
        payload: null,
      });
      let response = await axios.post(
        url,
        { leadFilter: payload.leadFilter },
        {
          headers: getHeaders(),
        }
      );
...
export const UPDATE_LEAD_STATUS = async (payload, leadId) => {
  let url = `${baseUrl}/api/student/leadUpdate/${getUserId()}/${leadId}`;
  let response = await axios.put(url, payload, { headers: getHeaders() });
  return response.data;
};
