...
// GetCandidate
export const GET_RECOMENDED_CANDIDATE = (type, pageNumber, perpage) => {
  return async (dispatch) => {
    try {
      let response = await axios.get(
        `${baseUrl}/api/dashboard/recomended_candidate/candidate_name/${localStorage.getItem("user_id")}/${type}/${pageNumber}/${perpage}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access_token")}`,
          },
