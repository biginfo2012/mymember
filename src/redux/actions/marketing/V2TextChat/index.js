...
import { GET_USER_INFORMATION } from "../../auth/loginActions";
import { SOCKET_CONNECTER_IO, SOCKET_EMIT_JOIN_ROOM_SYSTEM_TYPE, SOCKET_GET_NOTIFICATION_DATA, SOCKET_GET_TEXT_MESSAGES } from "../../socket.io";
...
export const SEEN_NOTIFICATION = (payload) => {
  return async (dispatch) => {
    try {
      let response = await axios.put(
        `${baseUrl}/api/notification_seen`,
        payload,
        {
          headers: getHeaders(),
        }
...
export const REMOVE_NOTIFICATION = (payload) => {
  return async (dispatch) => {
    try {
      let response = await axios.put(
        `${baseUrl}/api/notification_remove`,
        payload,
        {
          headers: getHeaders(),
        }
...
export const NOTIFICATION_ON_OFF = (params) => {
  let url = `${baseUrl}/api/notification_on_off/${getUserId()}`
  return async (dispatch) => {
    try {
