...
const ProjectListing = (props) => {
  const { taskFolderList, breadCrumbValue, GET_TASK_FOLDER_LIST, CREATE_TASK_SUB_FOLDER, ADD_BREAD_CRUMB, TASK_TO_DISPLAY_TO_USER,
    GET_TODAY_TASK_LIST, setFoldermenuOpen } = props
        ...
        taskFolderList?.map((item) => {
          return (
            <Fragment key={item?._id}>
              <ListItem
                button
                className={`${history.maintaskFolderId === item?._id ? classes.activeMainFolder : classes.inActiveFolder} pl-0`}
              >
                <Button
                  className={`${classes.folderBtn} pl-0`}
                  fullWidth
