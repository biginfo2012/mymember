                  ...
                  <div key={i} style={{ borderTop: "1px solid #dddddd" }}>
                    ...
                    <Collapse in={collapase === "pannel" + i}>
                      <div className="p-1">
                        ...
                        <NestedTable data={item?.stripe_history} selectedItem={item?.joinHistory} />
                      </div>
                    </Collapse>
                  </div>
...
const ChildStripeChange = (props) => {
  const classes = useStyles();
  const { stripeList, i, item, UPDATE_CANDIDATE_STRIPE, handleOpenCollapse, collapase, HandleselecteOne, CANDIDATE_REMOVE, SelectStudentForCandidate, rowIdWhichIsEnabled, setRowIdWhichIsEnabled, handleFilter } = props
  const [isDisable, setIsDisable] = useState(!(rowIdWhichIsEnabled === item?._id) || true)
  const [candidateVal, setCandidateVal] = useState(item?.candidate || stripeList && stripeList[0]?.candidate || "")
  ...
  useEffect(() => {
    setCandidateVal(item?.candidate || stripeList && stripeList[0]?.candidate || "")
  }, [item?.candidate, stripeList])

  useEffect(() => {
    setCurrStripeVal(item?.current_stripe)
  }, [item?.current_stripe, stripeList])
  ...
  return (
    <>
      <div className={classes.row} style={{ backgroundColor: isDisable ? "" : "rgba(1, 132, 255, 0.07)" }}>
        <div className={`${classes.displaybox}`}>
          <Checkbox
          ...
          >
            {
              stripes?.length > 0 ? (
                stripes?.map((stripe) => {
                  return (
                    <option value={stripe?.stripe_name} key={stripe?._id}>
                      {stripe?.stripe_name}
                    </option>
                  );
                }))
        ...
        <div className={classes.displaybox} style={{ marginLeft: "25px" }}>
          {
            isDisable !== true ?
              ...
              currentStripe === lastStripe ?
                isJoinedOrNotJoined ?
                  item?.joinHistory && item?.joinHistory[item?.joinHistory?.length - 1]?.join ?
                    <Chip
                      label={<b>Join</b>}
                      style={{
                  ...
                  <Chip
                    ...
                    onClick={() => {
                      ...
                      if (item?.joinHistory && item?.joinHistory[item?.joinHistory.length - 1]?.quite) {
                        setNotes(true)
                      } else {
                        setNotes(false)
                      }
