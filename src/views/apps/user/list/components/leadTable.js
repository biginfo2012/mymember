...
import ButtonDropDown from "../../../../ui-elements/selectDropdown/ButtonDropdown";
...
import "./index.css";
import { SOCKET_SEND_CHATBOT_MESSAGE } from "../../../../../redux/actions/socket.io";
...
const listsWithColor = [
  { key: "Cold", value: "Cold", color: "#00a6e169" },
  { key: "Warm", value: "Warm", color: "#d71a1c4d" },
  { key: "Hot", value: "Hot", color: "#ea5455a6" },
  { key: "Joined", value: "Joined", color: "#01a926" },
  { key: "Not Joined", value: "Not Joined", color: "#ef0000" },
];
...
const StudentTable = (props) => {
  ...
  }, [SELECTED_TEST_DATA, onSelectionChanged, selectedId, selectedRows]);
  ...
  const handleSelectRows = async (state) => {
    let _ids = getAllIds(state.selectedRows);
    setSelectedId(_ids);
  ...
  const RCcolumns = [
...

    {
      ...
      cell: (row) => (
        <ButtonDropDown
          leadId={row?._id}
          value={row?.leadStatus ? row?.leadStatus : "Cold"}
          items={listsWithColor}
        />
      ),
    },
    {
      ...
      sortable: true,
      cell: (row) => (
        <div>{row?.primaryPhone === "" ? "N/A" : row?.primaryPhone}</div>
      ),
    },

    {
      ...
      sortable: true,
      cell: (row) => <div>{row?.email === "" ? "N/A" : row?.email}</div>,
    },
